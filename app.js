...
// Route for a tutor's profile
app.get('/tutor/:name', (req, res) => {
    const tutorName = req.params.name;
    const tutor = tutors.find(t => t.name.replace(/\s+/g, '-') === tutorName);

    if (!tutor) {
        return res.status(404).send('Tutor not found');
    }

    // Render the tutor's profile using the EJS template
    res.render('tutor_profile', { tutor: tutor });
});
